-- 1)
/*Write a query that displays InvoiceId, CustomerId and total dollar 
...
-- 2)
/*Write a query that displays InvoiceId, CustomerId and total dollar 
amount for each invoice, but this time sorted first by total dollar amount 
...
-- 3)
/* Compare the results of these two queries above. How are the results different when you switch 
the column you sort on first? (Explain it in your own words.)*/

-- sıralama sütunlarında yazım önceliği olduğu görülüyor. Yukarıdaki iki soruda soldan
-- sağa  ilk yazılan sütuna göre sıralama yaptı.

--4)
/*Write a query to pull the first 10 rows and all columns from the invoices table 
...
-- 5)
/* Write a query to pull the first 5 rows and all columns from the 
...
--7)
/* Use the invoices table to find all information regarding invoices whose billing 
address is USA or Germany or Norway or 
Canada and invoice date is at any point in 2010, sorted from newest to oldest.*/

--SELECT * FROM invoices WHERE BillingCountry IN('USA', 'Germany', 'Norway')
